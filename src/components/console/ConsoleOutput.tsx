...
export function ConsoleOutput() {
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto p-2 font-mono text-xs">
      ...
      {logs.map((line, i) => (
        <pre key={i} className="whitespace-pre-wrap text-neutral-400">
          {line}
        </pre>
      ))}
      {running && <p className="text-neutral-500">Running…</p>}
      {error && (
      ...
      {hasResult && (
        <pre className="mt-2 whitespace-pre-wrap rounded bg-neutral-900 p-2 text-emerald-300">
